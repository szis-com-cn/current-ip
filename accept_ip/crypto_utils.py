...
class AuthenticationManager:
    ...
    def __init__(self, secret_key: str = None):
        ...
        Args:
            secret_key (str): 身份验证密钥
        """
        self.secret_key = secret_key
        self.crypto_manager = CryptoManager(secret_key) if secret_key else None
        self.logger = logging.getLogger(__name__)
    ...
    def is_encryption_enabled(self) -> bool:
        ...
        Returns:
            bool: 是否启用加密
        """
        return self.secret_key is not None and self.crypto_manager is not None
    
    def verify_request_authentication(self, headers: dict) -> bool:
        """
        验证请求头中的身份验证信息
        ...
        if not api_key:
            self.logger.warning("缺少 X-API-Key 请求头")
            return False
        
        # 验证 API 密钥
        if api_key != self.secret_key:
            self.logger.warning("X-API-Key 验证失败")
            return False
        
        return True
    
    def process_request_data(self, data: dict) -> tuple:
        """
        处理请求数据，根据是否加密进行相应处理
        ...
        Returns:
            tuple: (is_encrypted, processed_data, error_message)
        """
        try:
            # 检查是否为加密数据
            if 'encrypted_data' in data and 'signature' in data:
                if not self.is_encryption_enabled():
                    return False, None, "接收到加密数据但服务端未启用加密功能"
                ...
                if not self.crypto_manager.verify_signature(encrypted_data, signature):
                    return False, None, "数据签名验证失败"
                
                # 解密数据
                try:
                    decrypted_data = self.crypto_manager.decrypt_data(encrypted_data)
                    self.logger.info("成功解密请求数据")
                    return True, decrypted_data, None
                except ValueError as e:
                    return False, None, f"数据解密失败: {str(e)}"
            
            else:
                # 明文数据处理
                self.logger.info("处理明文数据")
                return False, data, None
                
        except Exception as e:
            return False, None, f"数据处理异常: {str(e)}"